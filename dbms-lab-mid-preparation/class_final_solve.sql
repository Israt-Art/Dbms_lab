...
IF EXISTS hr_schema;
	CREATE SCHEMA hr_schema COLLATE = utf8_general_ci;
...
/*QUERY 1 — Employees Who Earn More Than Their Manager’s Manager 
Problem: Find employees whose salary is greater than their manager’s manager,
provided that: • the employee belongs to a department, 
• the department has a location, 
• the location belongs to a region.
Output: employee id, first name, last name, salary */
...
WHERE e.salary > mm.salary;



/* QUERY 2 —Departments With Above-Average Salary Compared to Their Region 
Problem: Find departments whose average salary is greater than 
the average salary of all employees in the same region.
Output: department id, department name */
...
JOIN countries c ON l.country_id = c.country_id
JOIN regions r ON c.region_id = r.region_id
GROUP BY d.department_id, d.department_name, r.region_id
HAVING AVG(e.salary) >(
...
/* QUERY 3 —Employees Who Were Promoted but Never Changed Department 
Problem: Employee appears in job_history → they had at least one previous job.

Employee changed jobs at least once → promoted.

Employee never changed departments → stayed in the same department.

Employee currently earns more than all previous salaries → their current salary > all max salaries of previous jobs.

We want: employee_id, first_name, last_name. */
...
HAVING COUNT(DISTINCT jh.job_id) >= 2
    AND COUNT(DISTINCT jh.department_id) = 1
    AND e.salary > MAX(j.max_salary);

/*QUERY 4 —Departments With More Historical Employees Than Current Ones

We want departments where:

The number of distinct employees in job history > current employees in that department.

The region has more than one country.

We need: department_id, department_name.*/
SELECT 
    d.department_id,
    d.department_name
FROM departments d
...
GROUP BY d.department_id, d.department_name, r.region_id
HAVING 
    COUNT(DISTINCT jh.employee_id) > COUNT(DISTINCT e.employee_id)
    AND (
        SELECT COUNT(*)
        FROM countries c2
        WHERE c2.region_id = r.region_id
...
JOIN countries c ON l.country_id = c.country_id
JOIN job_history jh ON e.employee_id = jh.employee_id
GROUP BY e.employee_id, e.first_name, e.last_name, c.region_id
HAVING COUNT(jh.employee_id) >= 2 AND COUNT(DISTINCT jh.department_id) =
        (
            SELECT MAX(dept_count)
            FROM (
                SELECT 
                    e2.employee_id,
                    COUNT(DISTINCT jh2.department_id) AS dept_count
                FROM employees e2
                JOIN departments d2 
                ...
                JOIN countries c2 
                    ON l2.country_id = c2.country_id
