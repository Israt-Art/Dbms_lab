...
### 2)Use of DISTINCT
select employee_id,salary as month_salary,(salary/30) as salary_per_day
from employees;


### 3)This finds out the first and last name of those who has salary exactly 17000 or 24000.
...
### 6) Find employees’ employeeId, , firstName whose first names contains the string in
...
### 7)Find all employees employeeId, , firstName whose first names ends with string ‘el’ 
...
### 8)To find employees whose first names start with the letter T_m (eg. Tim. Tom 
### etc), end with the letter m, and contain any single character between. 
...
### 10)group by Count employees per department

select department_id,count(employee_id) as e_id
from employees
group by department_id;


### 11) Count employees per department who earns more than 10000
...
### 12)having  Find those departments that have more than 6 employees.
...
### 13)Find Max salary of the employees
...
### 14)  Using a subquery, list the name of the employees, paid more than 'Alexander' from emp_details 
### of the hr_schema database.
...
### 15) Suppose you want to find the employee id, first_name, last_name, and salaries for 
### employees whose salary is higher than the average salary throughout the company
...
### 16)Find the department names situated in Seattle
